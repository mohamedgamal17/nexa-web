...
@Component({
  ...
  imports: [CardModule, ButtonModule, SelectModule, InputTextModule, NgIcon, ReactiveFormsModule, FormsModule, CommonModule, TranslateModule, InputError],
...
export class ProfileAddress implements OnInit {
  ...
  currentCountry = computed(() => this.countries.find((x) => x.code == this.address()?.country));
  ...
  handleSubmit($event: SubmitEvent) {
    ...
    if (this.addressForm.valid) {
      if (this.hasAddressChanged(this.addressForm.value, this.address()!)) {
  ...
  hasAddressChanged(left: Address, right: Address) {
    return left.country != right.country || left.state != right.state || left.streetLine != right.streetLine || left.city != right.city || left.postalCode != right.postalCode || left.zipCode != right.zipCode;
  }
}
