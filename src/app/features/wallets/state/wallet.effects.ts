...
export class WalletEffects {
  ...
  loadWallets = createEffect(() => {
    return this.actions.pipe(
      ...
      mergeMap(({ paging }) =>
        this.userWalletService.getAllWallets({ skip: paging?.skip ?? 0, length: paging?.length ?? 10 }).pipe(
          map((resp) =>
          ...
          catchError((err) => of(walletActions.loadWalletsFailure({ error: err.error }))),
        ),
      ),
    );
  });
